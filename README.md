...
- **PWA:** vite-plugin-pwa + Workbox (service worker, precaching, update prompt)
...
The build generates a Workbox service worker (`sw.js`) that precaches all static assets. The build script automatically copies SW files to the Nitro output directory.
...
- **Offline-capable** — static assets (JS, CSS, HTML, images, fonts) are precached by the service worker
- **Auto-updates** — when a new version is deployed, users see a reload prompt. The service worker polls for updates hourly and the browser checks on navigation/focus
- **Standalone mode** — runs without browser chrome when installed (`display: standalone`)

### PWA Configuration

- **Service worker:** Generated by `vite-plugin-pwa` with Workbox `generateSW` mode
- **Manifest:** `public/site.webmanifest` (fallback) + plugin-generated `manifest.webmanifest`
- **Update strategy:** `registerType: "prompt"` — shows a toast-style prompt when a new version is available, user chooses when to reload
- **Icons:** 192x192 and 512x512 PNG icons in `public/`, plus maskable variant

### Testing the PWA locally

The service worker is **disabled in dev mode** (`devOptions.enabled: false`). To test PWA behavior:
